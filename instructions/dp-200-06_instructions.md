...
**랩 파일**: 이 랩의 파일은 _Allfiles\Labfiles\Starter\DP-200.6_폴더에 위치해있습니다.
...
디지털 혁신 프로젝트의 일환으로 CIO는 마케팅 부서가 업무의 측면을 통해 생산성을 높일 수 있도록 지원했습니다. 지난 몇 년 동안 마케팅 부서는 Twitter를 사용하여 판매되는 자전거 제품을 중심으로 마케팅 메시지를 증폭시켰습니다. 
...
1. Azure Portal에서 **리소스 만들기**를 선택하고 **이벤트 허브**를 입력한 다음 검색 결과에서 **이벤트 허브 네임스페이스**를 선택합니다.  그런 다음 **만들기**를 선택합니다.

1. 다음 옵션을 사용하여 새 이벤트 허브 네임스페이스를 생성한 다음 **만들기**를 클릭합니다.
...
1. 정책이 만들어지면 새 정책 **socialtwitter-eh-sap**를 클릭한 다음 **연결 문자열 - 기본 키**의 복사 버튼을 선택하고 연결 문자열 - 기본 키를 메모장에 붙여 넣습니다. 이 값은 이 연습에서 나중에 필요합니다.
...
## 연습 3: Stream Analytics 작업을 사용하여 데이터 처리
...
1. "프로젝트에 대해 알려주세요" 페이지에서 "관심 있는 사용 사례"에 대한 옵션을 선택합니다. 이 과정의 목적을 위해 **학생 프로젝트/코딩 학습**을 선택합니다. "무엇을 만들고 있는지 직접 설명해 주세요"에서 300단어 범위에서 사용 이유를 설명합니다. 다음은 예입니다.
...
1. 액세스 토큰 및 액세스 토큰 비밀 아래에서 **만들기** 버튼을 클릭합니다.

    >**참고**: 계정과 현재 애플리케이션에 대해 권한이 부여된 액세스 토큰 및 비밀이 생성됩니다.

1. 이전에 열었던 메모장 파일에 다음 키를 복사합니다.
    - 소비자 키(API 키)
    - 소비자 비밀(API 비밀)
    - 액세스 토큰
    - 액세스 토큰 비밀

### 작업 3: Twitter 클라이언트 애플리케이션 구성 및 시작
...
1. 메모장에서 데이터를 TwitterWPFClient.exe 애플리케이션에 입력합니다.
    ...
    - **Azure Event Hub 연결 문자열**: Endpoint=sb://xx-socialtwitter-eh.servicebus.windows.net/;SharedAccessKeyName=xx-social-eh-sap;SharedAccessKey=<메모장에서 CONNECTION STRING - PRIMARY KEY를 붙여 넣음>  
...
1. **TwitterWPFClient.exe**애플리케이션에서 **재생** 버튼을 클릭하고 콘솔에 트윗이 표시될 때까지 기다렸다가 애플리케이션을 실행 상태로 둡니다.

      > **참고**: **TwitterWPFClient.exe** 애플리케이션은 정의된 검색어에 대해 날짜, 검색어 감정 점수, Twitter 이름 및 트윗 세부 사항을 캡처합니다. 

1. 모든 애플리케이션을 계속 실행합니다.

### 작업 4: Stream Analytics 작업 프로비저닝
...
### 작업 5: Stream Analytics 작업 입력 지정
...
1. 이벤트 허브 화면에서 다음 값을 입력하고 **저장** 버튼을 클릭합니다.
...
1. 완료되면 TwitterStream 입력 작업이 입력 창 아래에 표시됩니다. 입력 창을 닫고 Streaming Analytics 작업 페이지로 돌아갑니다.

### 작업 6: Stream Analytics 쿼리 정의

1. 윈도우 중앙 **쿼리** 화면의 **socialtwitter-asa-job** 윈도우에서 **쿼리 편집**을 클릭합니다.
...
1. 쿼리 창을 닫고 검색 분석 작업 페이지로 돌아갑니다.


### 작업 7: Stream Analytics 작업 출력 지정
...
### 작업 9: Stream Analytics 작업 시작

1. **socialtwitter-asa-job** 창 중앙에 있는 **쿼리** 화면에서 **시작**을 클릭합니다.
...
1. JSON 파일이 나타나는지 확인하고 크기 열을 기록합니다.

1. 화면에서 파일 크기가 새로 고쳐지지 않은 경우 Microsoft Edge를 새로 고칩니다.
