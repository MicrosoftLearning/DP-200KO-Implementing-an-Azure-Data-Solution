...
7. **Azure Cosmos DB 계정 만들기** 블레이드에서 유효성 검사를 완료한 후에 **만들기**를 클릭합니다.
   ...
   > **참고**: 프로비전하는 데 약 5분이 소요됩니다. 이러한 랩에서 자주 건너뛰는 것은 Azure에서 서비스를 프로비전할 때 추가 탭에 대한 설명입니다. 프로비전 화면에 네트워크, 태그, 고급과 같은 추가 탭이 있습니다. 이를 사용하여 서비스에 대한 사용자 지정 설정을 정의할 수 있습니다. 예를 들어 여러 서비스의 네트워크 탭을 사용하면 가상 네트워크 구성을 정의할 수 있으므로 지정된 데이터 서비스에 대한 네트워크 트래픽을 제어하고 보호할 수 있습니다. 태그 옵션은 여러 리소스 및 리소스 그룹에 동일한 태그를 적용하여 리소스를 분류하고 통합 청구를 볼 수 있는 이름/값 쌍입니다. 고급 탭은 탭이 표시되는 서비스에 따라 다릅니다. 그러나 이러한 영역을 제어할 수 있으며 네트워크 관리자 또는 재무 부서와 협력하여 이러한 옵션을 구성하는 방법을 알아보려는 것이 중요합니다.
...
4. **컨테이너 추가** 블레이드에서 다음 설정을 사용하여 Clothing라는 컨테이너이 있는 Products 데이터베이스를 만듭니다.
...
3. 문서 창에서 **새 문서**의 아이콘을 클릭합니다. 이제 대체할 샘플 JSON과 함께 새 문서가 나타납니다.
...
6. 문서 창에서 **새 문서**의 아이콘을 클릭합니다.

7. 다음 코드를 복사하여 **문서** 탭에 붙여넣습니다.
...
8. 왼쪽 메뉴의 각 문서를 클릭하여 저장한 각 문서를 볼 수 있습니다. ID가 1인 첫 번째 항목의 값은 **33218896**이고(productid에 따라 명명) 두 번째 항목의 값은 **33218897**입니다.

### 작업 3: Azure Portal에서 쿼리를 실행합니다.

1. Edge 브라우저가 열리면 데이터 탐색기의 **항목** 화면에서 **SQL API** 블레이드 상단 **새로 고침** 아이콘 위의 **새 SQL 쿼리** 단추를 클릭합니다.

    > **참고**: **SELECT * FROM c** 쿼리가 표시된 쿼리 1 화면 탭이 나타납니다.

2. productId 1의 세부 정보를 나타내는 JSON 파일을 반환하는 쿼리를 변경합니다.
...
4. 기존 쿼리 창에서 productId에 대해 이전 쿼리 대신 ID, 제조업체 및 설명을 반환하는 쿼리를 JSON 파일에 작성합니다. 
...
1. Edge 브라우저가 열리면 데이터 탐색기의 **항목** 화면에서 쿼리 열기 단추 옆에 있는 **새 저장 프로시저** 단추를 클릭합니다.
...
7. Edge 브라우저가 열리면 데이터 탐색기에서 **새 저장 프로시저** 드롭다운 단추를 클릭하고 **새 UDF**를 클릭합니다.
...
2. 세계 지도에서 거주 중인 대륙 내의 데이터 센터 위치를 하나 한 번 클릭하고 **저장**을 클릭합니다.
...
2. **지역 읽기** 데이터 센터 위치를 클릭한 다음 "내용을 이해하며 현재 쓰기 영역에서 장애 조치(failover)를 트리거하는 데 동의합니다.” 옆의 체크박스를 클릭하고 **확인**을 클릭합니다.
...
4. "자동 장애 조치(failover)" 화면에서 **ON** 단추를 클릭한 다음 **확인**을 클릭합니다.
