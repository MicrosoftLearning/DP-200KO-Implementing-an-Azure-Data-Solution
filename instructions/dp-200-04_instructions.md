...
1. 새 블레이드에서 **마켓플레이스 검색** 텍스트 상자로 이동하여 **Cosmos** 라는 단어를 입력합니다. 표시되는 목록에서 **Azure Cosmos DB** 를 클릭합니다.

1. **Azure Cosmos DB** 블레이드에서 **만들기** 를 클릭합니다.
...
1. 새 저장 프로시저 화면에서 **사용자 정의 함수 ID** 텍스트 상자에 **producttax** 를 입력합니다.
...
1. Marketplace 텍스트 상자의 확장 검색에서 **Cosmos DB** 를 입력한 다음 **Azure Cosmos DB** 를 클릭합니다. 문서가 Visual Studio Code에 나타나면 **설치** 를 클릭합니다.
    ...
    > **참고**: 확장 프로그램이 설치되는 동안에는 이동 막대가 표시됩니다. 그런 다음 설치가 완료되면 바로 옆에 눈금이 나타납니다.
...
1. Visual Studio Code에서 **폴더 열기** 대화 상자가 나타납니다.

1. 원하는 위치에 **learning-module** 이라는 새 폴더를 만든 다음 **폴더 선택** 을 클릭합니다.

    > **참고**: 작업 폴더를 설정할 때 이 시점에서 Visual Studio Code가 다시 시작될 수 있습니다. 그러면 랩 문서를 다시 열어야 합니다.

1. Visual Studio Code의 왼쪽 메뉴에서 **Azure 아이콘** 을 클릭하고, **Cosmos DB** 아래에서 **구독** 을 마우스 오른쪽 단추로 클릭한 다음 **계정 만들기** 를 클릭합니다.

1. **새 Cosmos DB 계정 만들기** 화면의 상단에 있는 텍스트 상자에 **xxcosmos**(xx는 이니셜)라는 Azure Cosmos DB 계정의 고유한 이름을 입력한 다음 키보드에서 **Enter** 를 누릅니다.
...
1. 화면 상단의 입력 팔레트에서 데이터베이스 이름에 **Users** 를 입력하고 **Enter** 를 누릅니다.

1. 컬렉션 이름에 **WebCustomers** 를 입력하고 **Enter** 를 누릅니다.

1. 파티션 키에 **userId** 를 입력하고 **Enter** 를 누릅니다.

1. 마지막으로 초기 처리량 용량에서 **1000** 을 확인하고 **Enter** 를 누릅니다.
...
1. 터미널 프롬프트에서 다음 명령을 복사하여 붙여 **"Hello World!"** 프로그램을 실행합니다.

    ```bash
    dotnet run
    ```

1. 터미널 프롬프트에서 다음 명령 블록을 복사하여 붙여 필요한 NuGet 패키지를 설치합니다.
...
1. 터미널 화면에 패키지가 로드되고 명령이 표시됩니다(마지막은 **dotnet restore**), 터미널 창에서 **Enter** 를 누릅니다.
...
1. 왼쪽의 **Azure 아이콘** 을 클릭하고, 컨시어지 구독을 확장하고, **새 Azure Cosmos DB 계정을 마우스 오른쪽 단추로 클릭한 다음** **연결 문자열 복사** 를 클릭하여 연결 문자열을 복사합니다.

1. 연결 문자열을 App.config 파일의 끝에 붙여넣은 다음 연결 문자열에서 **AccountEndpoint** 부분을 App.config의 **accountEndpoint** 값에 복사합니다.

1. 이제 연결 문자열에서 **AccountKey** 값을 **accountKey** 값으로 복사한 다음 복사한 **원래 연결 문자열을 삭제** 합니다.
...
1. 터미널 프롬프트에서 다음 명령을 복사하여 붙여 프로그램을 실행합니다.

    ```bash
    dotnet run
    ```

    > **참고**: 이 프로그램은 터미널에서 Hello World를 표시합니다!

1. 탐색기 창에서, Program.cs를 클릭하여 파일을 엽니다. Program 클래스의 시작 부분에 다음 코드를 추가합니다.
...
1. 새 클라이언트를 만드는 비동기 태스크를 새로 추가하고, Main 메서드 다음에 아래 메서드를 추가하여 Users 데이터베이스가 있는지 확인합니다.
    ...
    private async Task BasicOperations()
    {
        ...
        Console.WriteLine("데이터베이스 및 컬렉션 유효성 검사 완료");
    }
    ```

1. 통합 터미널에서 다시 다음 명령을 복사하여 붙여 프로그램을 실행하여 코드가 실행되는지 확인합니다. 콘솔에 "Hello World!"가 반환됩니다.
...
1. 다음 코드를 복사하여 **BasicOperations** 메서드의 끝에 붙여 넣습니다.
...
1. 화면 상단의 텍스트 상자에서 **UpdateOrderTotal** 을 입력하고 Enter를 눌러 저장 프로시저의 이름을 지정합니다.

    > **참고**: 이전 단계에서 다음 단계를 수행하지 않았다면 기본적으로 첫 번째 항목을 검색하는 저장 프로시저가 제공됩니다.
