...
이 랩에서는 학생이 주어진 비즈니스 및 기술 요구 사항에 대해 구현할 적절한 스토리지 유형을 결정할 수 있습니다. Azure Storage 계정 및 Data Lake Storage 계정을 만들고 Data Lake Storage 버전1과 버전2의 차이점을 설명할 수 있습니다. 또한 선택한 데이터 스토리지에 데이터 로드를 수행하는 방법을 시연할 수 있습니다.
...
> **중요**: 이 랩을 진행하면서 프로비전 또는 구성 작업에서 발생한 문제를 기록하고 _\Labfiles\DP-200-Issues-Doc.docx_ 에있는 문서의 테이블에 로그인합니다. 랩 번호를 문서화하고, 기술을 기록하며, 문제를 설명하고, 해결 방법이 무엇인지 설명합니다. 이 문서를 나중에 모듈에서 다시 참조할 때 저장합니다.
...
2. **10분**을 할애하여 이 랩의 시나리오에 설명된 대로 데이터 저장소 요구 사항을 문서화합니다. 추가 참조를 위해 사례 연구 문서를 사용할 수도 있습니다.
...
2. 랩 위치에 가장 가까운 지역에서 리소스 그룹 awrgstudxx 내에 스토리지 계정 **awsastudxx**를 만들고 구성합니다. 여기서 **xx**는 사용자의 이니셜입니다.
...
2. Azure Portal에서 **리소스 그룹**을 클릭합니다.
...
4. 리소스 그룹 만들기 화면에서 **검토 + 만들기를** 클릭합니다.
...
1. Azure Portal에서 _배포가 완료되었습니다_. 메시지가 표시됩니다. 그러면 **리소스로 이동** 단추를 클릭합니다.

2. **awsastudxx** 화면의(**xx**는 사용자 이니셜) **Blob 서비스**에서 **컨테이너**를 클릭합니다.
...
6. 4~5단계를 반복하여 **비공개(익명 액세스 없음)** 공용 액세스 수준으로 **phonecalls**라는 컨테이너를 만듭니다.

7. 4~5단계를 반복하여 **비공개(익명 액세스 없음)** 공용 액세스 수준으로 **tweets**라는 컨테이너를 만듭니다. 화면이 아래 그래픽과 같이 나타나야 합니다.
...
7. **Blob 업로드** 화면을 닫고 **images**화면을 닫습니다.
...
2. awsastudxx 스토리지 계정 내에서 **logs** 및 **data**라는 컨테이너를 만듭니다.
...
8. **스토리지 계정 만들기*** 블레이드를 유효성 검사한 후, **만들기**를 클릭합니다.
...
2. **awdlsstudxx** 화면에서(**xx**는 사용자의 이니셜) **컨테이너**를 클릭합니다.
...
8. 설치가 완료되면 **Microsoft Azure Storage Explorer 시작** 옆의 체크박스가 선택되어 있는지 확인한 다음 **마침**을 클릭합니다. Microsoft Azure Storage Explorer가 열리고 구독이 나열합니다.
...
2. **스토리지 계정** 아래에서 스토리지 계정 **awdlsstudxx(ADLS Gen2)** 를 검색하고 화살표를 클릭하여 확장합니다.
...
9. **Blob 컨테이너** 아래에서 화살표를 클릭하여 확장하고 **data**파일 시스템을 표시합니다. **data** 파일 시스템을 클릭합니다.
